...
namespace HuaTianProject.Core
{
    ...
    public class DBHelper
    {
        ...
        public static SQLiteCommand CreateCommand(SQLiteConnection connection, string commandText,
            params SQLiteParameter[] commandParams)
        {
            SQLiteCommand cmd = new SQLiteCommand(commandText, connection);
            if (commandText.Length > 0)
            {
                foreach (SQLiteParameter parm in commandParams)
                {
