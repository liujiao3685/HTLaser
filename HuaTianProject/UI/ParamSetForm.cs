                    ...
                    for (int col = 0; col < dataGridView1.ColumnCount; col++)
                    {
                        switch (col)
                        {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                dataGridView1.Rows[row].Cells[col].Value = Convert.ToDouble(item.ParamValue).ToString("0.00");
                                break;
                            case 4:
            ...
            foreach (var item in dic)
            {
                ...
                switch (item.Key)
                {
                    case "MoveSpeed":
                        ...
                        string s = Convert.ToDouble(item.Value).ToString("0.00");
                        param.ParamValue = Convert.ToDouble(s);
                        param.ParamName = "运动速度";
                        param.ParamUnit = "mm/s";
                        break;
                    case "PulseEquiv":
                        param.MaxValue = MaxValue;
                        param.MinValue = MinValue;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int row = 0; row < dataGridView1.RowCount; row++)
                {
                    ...
                    {
                        case "运动速度":
                            FormMain.MoveSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                        ...
                        case "脉冲当量":
                            ...
                            break;
                        case "回原点速度":
                            FormMain.HomeSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                            FormMain.XmlData.Add("HomeSpeed", FormMain.HomeSpeed.ToString());
                            break;
                        case "相对速度":
                            FormMain.AbsoulteSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                            FormMain.XmlData.Add("AbsoulteSpeed", FormMain.AbsoulteSpeed.ToString());
                            break;
                        case "绝对速度":
                            FormMain.ReletiveSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                            FormMain.XmlData.Add("ReletiveSpeed", FormMain.ReletiveSpeed.ToString());
                            break;
                        case "JOG速度":
                            FormMain.JogSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                            FormMain.XmlData.Add("JogSpeed", FormMain.JogSpeed.ToString());
                            break;
                        case "圆弧插补速度":
                            FormMain.ArcInterSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
                            FormMain.XmlData.Add("ArcInterSpeed", FormMain.ArcInterSpeed.ToString());
                            break;
                        case "直线插补速度":
                            FormMain.LineInterSpeed = Math.Round(Convert.ToDouble(dataGridView1.Rows[row].Cells["colValue"].Value), 2);
